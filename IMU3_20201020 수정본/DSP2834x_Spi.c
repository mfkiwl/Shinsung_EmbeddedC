...
/*
	SpidRegs.SPICCR.all = 0x0000;
	SpidRegs.SPICTL.all = 0x0000;
	SpidRegs.SPIBRR = 0x0000;
	SpidRegs.SPIPRI.all = 0x0000;

	SpidRegs.SPICCR.bit.SPISWRESET=0;				// Reset SPI
	SpidRegs.SPICCR.bit.CLKPOLARITY = 0;			// SPI_LOAD_RASIONG

	SpidRegs.SPICCR.bit.SPICHAR = 15;				// 8bit 데이터 크기 사용
	SpidRegs.SPICTL.bit.MASTER_SLAVE = 1;			// 마스터 모드
	SpidRegs.SPICTL.bit.TALK = 1;					// 통신 활성화
	SpidRegs.SPICTL.bit.CLK_PHASE = 1;

	SpidRegs.SPIBRR = 29;							// 10Mbps
...
// Comment out other unwanted lines.
	...
	GpioCtrlRegs.GPBMUX2.bit.GPIO51 = 3; // Configure CS

	GpioCtrlRegs.GPBDIR.bit.GPIO51 = 1;  // GPIO51 is output
