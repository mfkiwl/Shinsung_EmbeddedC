...
// GLOBAL VAL
double R_latlon[2][2]; //32bit
double R_PV[6][6];
double R_zupt[3][3];	   // 10.19.13:22
double R_zupt2[3][3];	   //10.20.10:24
double R_latlonvnve[4][4]; //
double P_DB[15][15];
double Qvec[6];
double H_latlon[2][15];
double H_PV[6][15];
double H_latlonVnVe[4][15]; // 10.20.10:43
double H_Zupt[3][15];		// 10.19.13:22
double H_hgt[1][15];
double F[15][15];
double PHI[15][15];
double R_hgt[1][1];
...
double Fb[3], Wbib[3], fn[3] = {0};
// 위치,속도만 사용하는 경우, 위도,경도만 사용하는 경우, 고도만 사용하는 경우 EKF의 측정치 (delta 측정치)
double MeasurementPV[6], Measurementlatlon[2] = {0}; // 중력 벡터   // 10.20.10:40
double Measurementlatlonvnve[4] = {0};
double MeasurementV[3] = {0}; //10.19.13:28
double g[3] = {0};
...
double time_p, fb[3], wbib[3], gps_data[7], Baro_data;
// 정렬에 사용되는 변수, 기압계 데이터가 전체적으로 bias되어 있어 이를 고려함.
double mean_acc[3], mean_gyr[3] = {0}, mean_BaroOffset, roll_init, pitch_init, yaw_init = 0;
// mean_BaroOffset을 계산하기 위해 사용되는 변수
double gps_tmp = 0;
double baro_tmp = 0;

double parsedIMUGPS[SPI_RX_BUFF_LEN / 2];
double parsedPrevious[SPI_RX_BUFF_LEN / 2];
Uint16 prevGPS[2];
double gpsvel_N = 0; //
double gpsvel_E = 0; //
...
void read_data()
{
	...
	{
		parsedIMUGPS[11] = (strtod("37.373737", &pEnd));
		parsedIMUGPS[12] = (strtod("126.126126", &pEnd));
		parsedIMUGPS[13] = (strtod("39.30", &pEnd));
		parsedIMUGPS[14] = (strtod("0.07", &pEnd));
		parsedIMUGPS[15] = (strtod("07", &pEnd)); /////////////////////////////*** 방향각도 들어오고있는지??
	}
...
void init_data_00()
{
	memset(R_latlon, 0x0, sizeof(double) * 2 * 2);
	memset(R_PV, 0x0, sizeof(double) * 6 * 6);
	memset(R_latlonvnve, 0x0, sizeof(double) * 4 * 4); //
	memset(P_DB, 0x0, sizeof(double) * 15 * 15);
	memset(R_zupt, 0x0, sizeof(double) * 3 * 3);  //10.19.13:24
	memset(R_zupt2, 0x0, sizeof(double) * 3 * 3); //10.20.10:25
	memset(Qvec, 0x0, sizeof(double) * 6);
	memset(H_latlon, 0x0, sizeof(double) * 2 * 15);
	memset(H_PV, 0x0, sizeof(double) * 6 * 15);
	memset(H_Zupt, 0x0, sizeof(double) * 3 * 15); // 10.19.13:28
	memset(H_hgt, 0x0, sizeof(double) * 1 * 15);
	memset(F, 0x0, sizeof(double) * 15 * 15);
	memset(PHI, 0x0, sizeof(double) * 15 * 15);
	memset(R_hgt, 0x0, sizeof(double) * 2); // 10.20.10:52 2가 아니라 1아닌지?(porting 과정에서 오류?)

	memset(INSRadPos, 0x0, sizeof(double) * 3);
	memset(INSDegPos, 0x0, sizeof(double) * 3);
	memset(INSRadEulr, 0x0, sizeof(double) * 3);
	memset(INSDegEulr, 0x0, sizeof(double) * 3);

	memset(INSQT, 0x0, sizeof(double) * 4);
	memset(INSDCMCnb, 0x0, sizeof(double) * 3 * 3);
	memset(INSDCMCbn, 0x0, sizeof(double) * 3 * 3);
	memset(INSMpsVned, 0x0, sizeof(double) * 3);
	memset(INSMpsVb, 0x0, sizeof(double) * 3);

	memset(Ba_init, 0x0, sizeof(double) * 3);
	memset(Bg_init, 0x0, sizeof(double) * 3);

	memset(Ba, 0x0, sizeof(double) * 3);
	memset(Bg, 0x0, sizeof(double) * 3);

	memset(wnin, 0x0, sizeof(double) * 3);
	memset(wnie, 0x0, sizeof(double) * 3);

	memset(wnen, 0x0, sizeof(double) * 3);
	memset(wbin, 0x0, sizeof(double) * 3);
	memset(wbnb, 0x0, sizeof(double) * 3);

	memset(estX, 0x0, sizeof(double) * 15);

	memset(Fb, 0x0, sizeof(double) * 3);
	memset(Wbib, 0x0, sizeof(double) * 3);
	memset(Measurementlatlonvnve, 0x0, sizeof(double) * 4); //
	memset(fn, 0x0, sizeof(double) * 3);
	memset(MeasurementV, 0x0, sizeof(double) * 3); // 10.19.13:30
	memset(MeasurementPV, 0x0, sizeof(double) * 6);
	memset(Measurementlatlon, 0x0, sizeof(double) * 2); // 10.20.10:42

	memset(g, 0x0, sizeof(double) * 3);
	...
	memset(fb, 0x0, sizeof(double) * 3);
	memset(wbib, 0x0, sizeof(double) * 3);
	memset(gps_data, 0x0, sizeof(double) * 7);

	memset(mean_acc, 0x0, sizeof(double) * 3);
	memset(mean_gyr, 0x0, sizeof(double) * 3);
	...
	memset(parsedIMUGPS, 0x0, sizeof(double) * (SPI_RX_BUFF_LEN / 2));
	memset(parsedPrevious, 0x0, sizeof(double) * (SPI_RX_BUFF_LEN / 2));
...
void init_data_01()
{
	...
	double R_vec4[3] = {noise_v, noise_v, noise_v};											 //10.20.10:26
	double R_vec5[3] = {noise_v * 2, noise_v * 2, noise_v * 2};								 //10.19.13:30
	...
	diag(R_vec4, 3, (double *)R_zupt);	// 10.19.13:30
	diag(R_vec5, 3, (double *)R_zupt2); //10.20.10:26
}
...
int pre_process()
{
	...
	memset(C1T, 0x0, sizeof(double) * 3 * 3);
	memset(C1T_T, 0x0, sizeof(double) * 3 * 3);
	memset(omega_tmp, 0x0, sizeof(double) * 3);
...
int align_process()
{
	...
	double measurement1[6];
	double measest1[6];
	double measurement2[3]; //10.20.10:47
	double measest2[3];		//10.20.10:47
	double measest4[4];		//////////////////////////////////////////////**
	double measurement3[2]; //
	double measurement4[4];
	double measest3[2];

	double MeasurementHgt[1];
	...
	memset(delMpsVned, 0x0, sizeof(double) * 3);
	memset(wnie2tmp, 0x0, sizeof(double) * 3);
	memset(coriolitmp, 0x0, sizeof(double) * 3);

	VecScalarMult(wnie, 2, wnie2tmp, 3);
	MatSum(wnie2tmp, wnen, 3, 1, coriolitmp);

	memset(coriolitmp2, 0x0, sizeof(double) * 3);
	...
	memset(delvdt, 0x0, sizeof(double) * 3);
	...
	// processed data
	sprintf(uart_buf, "P,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e\r\n",
...
#if 0
	sprintf(uart_buf, "baud -s:115200\r\n");
	scia_msg(uart_buf);
	sprintf(uart_buf, "open -f:/log.txt\r\n");
	scia_msg(uart_buf);
#endif
